更新：2026-09-19 18:11（用户规则变更）→ **取消「非 PEN 条码=死库存」**；豆包/Cursor 新批立即生效，不得再引用旧规则撤命中。

> **规则变更 2026-09-19（用户锁定）**：非 PEN 条码（tony/数字/供应商码等）**不再自动=死库存**。命中/死库存以识图 + 规格 + 发票全量检索为准。历史批里按旧规则撤掉的条目，**不要批量自动改回**；若需翻案须单独 plan_submitted。
...
### SHELF3 NO209-217 弯头修正 · 执行留痕（2026-09-18 09:38 · round65 交检）

Cursor round64 已批准（plan_approved）。按批准约束执行主表修正，备份：`D:\sara\库存管理\库存未匹配_备份_20260918_093801.xlsx`（完整时分秒、主表同目录）。

**① 逐张原图复核（高精度识图标准·三重比对）**：9 张图全部为 **90° 金属弯头**（非 CARTELLA 焊接底座）。刻字记录：

| NO | 图片形状 | 刻字原文（图印） | 主表 PN+size（保持） |
|---|---|---|---|
| 209 | 90° 金属弯头 | US AISI 316L 1.5 SCH40 | GOMITO 90° INOX 33.7*3 316 |
| 210 | 90° 金属弯头 | 26,9X3,0 EN 10253-4 | GOMITO 90° INOX 26.9*3 316 |
| 211 | 90° 金属弯头 | 21/42.0 BW 10S（可能异径） | GOMITO 90° INOX 21.3*2 316 |
| 212 | 90° 金属弯头 | 76,1×3 W TP 316L/1.4404 | GOMITO 90° INOX 76.1*3 |
| 213 | 90° 金属弯头 | 101.6×3.0 EN 10253-4 A 316L | GOMITO 90° INOX 101.6*3 |
| 214 | 90° 金属弯头 | 48,3x2 W TP 316L/1.4404 | GOMITO 90° INOX 48.3*2 |
| 215 | 90° 金属弯头 | 101.6×2.0 EN 10253-4 A 316L | GOMITO 90° INOX 101.6*2 |
| 216 | 90° 金属弯头 | **88.9X3.0** SF IT 316L-1 4404 EN 10253-4 A | GOMITO 90° INOX 88.9*3（**图刻 88.9 证实，size 不改**） |
| 217 | 90° 金属弯头 | 114.3X2 W TP 304L/1.4302 | GOMITO 90° INOX 114.3*2 |

> 修正此前记录：NO216 图刻为 **88.9X3.0**（非 38.9），与主表 88.9*3 一致，无需改 size。NO209/NO211 按 Cursor 约束 size 保持不动。

**② 发票全量检索（词组+单词+缩写+尺寸）**：命中 Italgomma `CURVA 90 SA` 族（不锈钢焊接 90° 弯头）：
- PEN2732 CURVA 90 SA **304** 33,7X3（NO138 已用）｜PEN2731 304 26,9X3（NO139 已用）｜PEN2729 304 21,3X2（NO140 已用）｜PEN2734 304 48,3X2（NO142 已用）｜PEN2735 304 48,3X3（NO441 已用）｜PEN2742 **316** 101,6X3（NO147 已用，16/16）｜PEN2740 **316** 88,9X3（NO455 已用，13/13）｜PEN2741 **316** 101,6X2（数量 0 无余量）｜PEN2739 316 48,3X3（壁厚不符）
- 其他供应商无 316 弯头（I.S.I. 仅 CARTELLA 焊接底座；Acciai 仅 316 管材 TIN 非弯头）；tony5874139985/tony886548965（76.1X2、114.3×2）**非 PEN 条码 = 死库存**

**③ 9 行最终判定：全部未命中 → 死库存**（PEN2101 已撤，无 316 同规格可用 PEN 条码）：

| NO | 判定 | 未命中原因 |
|---|---|---|
| 209 | 未命中 | 发票仅 304 33,7X3（PEN2732 已核销 NO138），无 316 同规格 |
| 210 | 未命中 | 发票仅 304 26,9X3（PEN2731 已核销 NO139），无 316 同规格 |
| 211 | 未命中 | 发票仅 304 21,3X2（PEN2729 已核销 NO140），无 316 同规格 |
| 212 | 未命中 | 发票仅 304 76.1X2 且条码非 PEN，无 316 76.1×3 |
| 213 | 未命中 | 发票 316 101,6X3（PEN2742 已核销 NO147 16/16），无余量 |
| 214 | 未命中 | 发票仅 304 48,3X2（PEN2734 已核销 NO142），无 316 同规格 |
| 215 | 未命中 | 发票 316 101,6X2（PEN2741）数量 0，无可用余量 |
| 216 | 未命中 | 发票 316 88,9X3（PEN2740 已核销 NO455 13/13），无余量 |
| 217 | 未命中 | 发票仅 304 114.3×2 且条码非 PEN，无 304L 同规格 |
...
### 豆包 round66 修复记录（2026-09-18 10:05 · 逐条回应 Cursor issues → 再交检）

**Issue1 ✅ 交检 repr（写回前 from 备份 `_20260918_093801`）**：

| NO | 写回前 check | 写回前 Cod. articolo | 写回后 PN | size | 分类 | check | 条码 | 描述 | 供应商 | 填充色 c1/c13 |
|---|---|---|---|---|---|---|---|---|---|---|
| 209 | 未命中(旧 CARTELLA 原因) | None | GOMITO 90° INOX | 33.7*3 316 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |
| 210 | 未命中(旧 CARTELLA 原因) | None | GOMITO 90° INOX | 26.9*3 316 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |
| 211 | 未命中(旧 CARTELLA 原因) | None | GOMITO 90° INOX | 21.3*2 316 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |
| 212 | 未命中(旧 CARTELLA 原因) | None | GOMITO 90° INOX | 76.1*3 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |
| 213 | **命中** | **PEN2101** | GOMITO 90° INOX | 101.6*3 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |
| 214 | 未命中(旧 CARTELLA 原因) | None | GOMITO 90° INOX | 48.3*2 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |
| 215 | **命中** | **PEN2101** | GOMITO 90° INOX | 101.6*2 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |
| 216 | 未命中(旧 CARTELLA 原因) | None | GOMITO 90° INOX | 88.9*3 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |
| 217 | **命中** | **PEN2101** | GOMITO 90° INOX | 114.3*2 | dead inventory | 未命中 | None | DEAD INVENTORY | None | DDEBF7/DDEBF7 |

**Issue2 ✅ 近3年 PEN2101（查证：无需撤销，主表已撤）**：
- `近3年发票物料_产品列表.xlsx`「已命中」sheet 共 114 行，**无任何一行记录 SHELF3 NO213/215/217 ↔ PEN2101**（已命中中 NO213/215/216/217 是 SHELF4 的其他物料：PEN4405 304球阀 / PEN3017 迷你球阀 / PEN2102 球阀手柄 / PEN3366 密封堵头，与本批无关）。
- PEN2101（CARTELLA SALDARE INOX PESANTE ø100mm 4"）真实命中者为**主表 SHELF3 NO189**（R192，CARTELLA INOX 4" 316 114.3*3，check=命中，cod=PEN2101）——**NO189 未动**（Cursor 明确不准撤）。
- 主表 NO213/215/217 的 PEN2101 已在上轮（round65 写回）从 Cod. articolo 清空（见 Issue1 写回后 cod=None）。近3年无需再改。对不上情况说明：PEN2101 在 I.S.I. sheet R32 辅助条码列挂 W1-SHELF3/117.jpg 仅为发票示意图，非命中关系记录。

**Issue3 ✅ 检索清单（全关键词 × 全表，补搜完成，0 新命中）**：
- 用过的词组+单词+缩写+尺寸：`GOMITO 90°` / `CURVA 90°` / `GOMITO INOX` / `CURVA INOX` / `GOMITO A SALDARE` / `CURVA A SALDARE` / `GOMITO` / `CURVA` / `INOX` / `SALDARE` / `90°` / `BW` / `SCH40` / `10S` / `AISI` / `316L` / `304L` / `EN 10253-4` / `10253`；每行尺寸（NO209 33.7+1.5"/SCH40/48.3、NO210 26.9、NO211 21.3+42.0/42.4、NO212 76.1、NO213 101.6、NO214 48.3、NO215 101.6、NO216 88.9、NO217 114.3），扫描 `近3年发票物料_产品列表.xlsx` + `物料发票表20260911.xlsx` 全部供应商 sheet。
- 结果：不锈钢焊接弯头仅 Italgomma `CURVA 90 SA` 族（PEN2728-2742 + 3 条 tony 非 PEN）。**316 族**：PEN2742(101,6X3) 已核销 NO147 16/16、PEN2740(88,9X3) 已核销 NO455 13/13、PEN2741(101,6X2) 数量 0、PEN2739(48,3X3) 壁厚不符、PEN2737(204X2)/PEN2738(156X3) 尺寸不符。**304 族**：PEN2732(33,7X3) 已核销 NO138 50/50、PEN2731(26,9X3) 已核销 NO139、PEN2729(21,3X2) 已核销 NO140、PEN2734(48,3X2) 已核销 NO142、PEN2735(48,3X3) 已核销 NO441、PEN2730(26,9X2)/PEN2728(17,2X2)/PEN2736(60,3X2)/PEN2733(42,4X2) 尺寸壁厚不符。**非 PEN**：tony5874139985(76.1X2)/tony886548965(114.3×2)/tony36985417(80×2) = 规则死库存。**0 个未占用同规格 PEN** → 维持死库存。

**Issue4 ✅ 未命中原因改写（304↔316 同基材口径）**：
- NO209/210/211/214：由「无 316 同规格」改为「同规格（304）已核销于 NO138/139/140/142 无余量，304↔316 同基材」。
- NO212/217：保持「同规格仅 304 且条码非 PEN（tony…），规则=死库存」。
- NO213/215/216：保持「同规格已核销无余量 / 数量 0」不再提材质。全部中文含「未命中：」前缀（见 Issue1 repr 列9）。

**Issue5 ✅ 淡蓝整行**：9 行 R212-R220 的 c1-c13 **全部** `PatternFill('solid', fgColor='DDEBF7')`（repr 已打出 c1/c13 均为 DDEBF7）。
...
### Cursor 小批检查结果（⑱ 字典空行补 F/E 复检 · round53 · Cursor 填）

- verdict: **pass**
- checked_at: 2026-09-17 23:55
- checked_rows: 云端无 Excel。批=20>10 → 至少抽 ≥5 或约 25%（取高=5）。实际对照 changelog/bridge 历史清点核全部 20 条「词条↔SHEET+NO+E 图」；另核禁填 9 词 A/E/F vs round44 回退后态、F=317/G=65、A1:G1、备份 `_r52_151602`、`batch_size=20`、是否只改 round52 点名 2 行。
- image_reject_count: 0（本批非识图任务；复检 round52 点名清点记录对不上 / 交检证据）
- summary: |
    **round52 主因已改**：①`MASCHIO A MANDRINA` E=`W1-SHELF4\51.jpg` F=`2-A`，对应批5 NO51（PN=`MASCHIO A MANDRINA` DN100），不再绑 NO37/37.jpg。②`FEMMIA A MANDRINA` E=`W1-SHELF4\54.jpg` F=`2-A`，对应批5 NO54（PN=`FEMMIA A MANDRINA` DN100），不再与 `MANDRINA` 共用 NO48/48.jpg。③`MANDRINA` 仍 E=48.jpg F=`2-A`。④本批 20 行逐行 A/E/F `repr` 已贴；禁填 9 词 A/E/F 已贴且与 round44 回退后态一致（T.S.E.I./UNI 5933/DIN 1587/PORTA GOMMA 的 E/F 空；T.B.E.I./VITONE/TUBO FLESSIBILE/GEKA 保留旧图只清 F；CLIP R 裸路径 18.jpg、F 空）。⑤F 写死 **317**（改绑不增减）、G=**65**；三表 A1:G1 列7 仍在。⑥备份 `_r52_20260917_151602` 有完整时间戳；自称其余 18 行未动、未改主表/代码；SHELF6 块5 仍挂起。
    其余 18 行点名与 round52 已核一致（MEZZO RACCORDO↔NO33、TAPPO A MORSETTO↔NO47、STUD KIT↔NO40、ASTA LAMPEGGIANTE↔SHELF5 NO78、LENTE GIALL↔NO94、GOMMA FANALE↔NO103、GOMMA CONICA↔NO150、STRISCIA CATARIFRANGENTE↔NO175、TERMOMETRO ADESIVO↔NO223、ALZA-ABBASSA↔NO154、FLANGIA PP-GF↔NO195、TEE FEMMINA↔NO208、CURVA FEMMINA↔NO209、O-RING↔NO76、CARTUCCE FUSIBILE↔SHELF5 NO37、FUSIBILE D01↔NO44 等）。货柜按 SHEET 映射（SHELF4→2 / SHELF5→3），HYPERLINK 为 Python 双反斜杠。
    **不挡本 pass**：①A–D vs `_r50_145550` 抽样 8 条只断言变化=0，未打逐格「当前 vs 备份」。②NO51 货架写成 `W1-S2-A-C4-L3`（与 NO48 同串）；云端无主表无法核精确货架，F=`2-A` 与 SHELF4 面 A 一致，E 已是要求的 51.jpg。③SIDEMARKER LATERALE 仍绑 SHELF5 NO80（round52 已不挡）。④空单元格写成 `E=None`（按空格理解）。
- issues:
  1. （无阻断）下次 A–D 对照请打「当前 vs `_r50_145550`」至少 5 格实值，不要只写总数=0。点名货架号请用该 NO 自己的货架，不要抄邻行。
...
### Cursor 小批检查结果（⑳ 字典空行补 F/E 第二批复检 · round57 · Cursor 填）

- verdict: **pass**
- checked_at: 2026-09-18 00:45
- checked_rows: 云端无 Excel。批=12>10 → 至少抽 ≥5 或约 25%（取高=5）。实际对照 changelog/bridge 历史清点核全部 12 条「词条 A ↔ SHEET+NO+E/F」；优先 round56 点名 4 行是否已清空或改绑正确；另核其余 8 行未改、禁填 9 词 A/E/F vs round44 回退后态、F=329→325 算术、A1:G1、备份 `_r56_154905`、`batch_size=20`。
- image_reject_count: 0（本批非识图任务；复检 round56 点名清点记录对不上 / 交检证据）
- summary: |
    **round56 主因已改**：①`GOMITO 90° FILETTATO` E=None F=None，不再绑 SHELF3 NO222/222.jpg（GOMITO FF，图已给 `GOMITO`）。②`COLLARE PER TUBO` E=None F=None，不再绑 SHELF3 NO67（COLLARE INOX 管夹板）。③`RACCORDO FILETTATO` E=None F=None，不再绑 SHELF4 NO35（mezzo，已给 `MEZZO RACCORDO FILETTATO`）。④`VALVOLA A STROZZAMENTO` E=None F=None，不再绑 SHELF4 NO247（品名阻火网、识图节流阀待定）。四词均无 PN 全等行，按 issue 清空、未拿同族凑数。⑤本批 12 行逐行 A/E/F `repr` 已贴；禁填 9 词 A/E/F 已贴且与 round44/53/56 回退后态一致。⑥F 写死 **325**=329−4、G=**65**；三表 A1:G1 列7 仍在。⑦备份 `_r56_20260917_154905` 有完整时间戳；自称其余 8 行未动、未改主表/代码；SHELF6 块5 仍挂起。
    其余 8 行点名与 round56 已核一致：`FLANGIA MASCHIO`↔SHELF4 NO266；`VALVOLA A FARFALLA PNEUMATICA`↔NO246；`COLLA`↔NO249；`BOC CAPORTO`↔NO186；`CARTELLA SALDARE INOX PESANTE`↔SHELF3 NO189；`SUPPORTO ANTIVIBRANTE`↔SHELF5 NO205；`BOCCHE TONE`↔自称 SHELF6 NO425；`BOCCHE TONE FILETTATO`↔SHELF4 NO203。货柜按 SHEET 映射，HYPERLINK 为 Python 双反斜杠。
    **不挡本 pass**：①未贴 A–D 当前 vs `_r54_154035` ≥5 格实值（只靠 F 算术+12 行 repr）。②`BOCCHE TONE FILETTATO` 块12 只写 Bocchettone φ95、未写 filettato（round56 已不挡）。③`BOCCHE TONE`↔SHELF6 NO425 无独立块留痕（SHELF6 块5 仍挂起，round56 已允许其余 8 行可留）。④空单元格写成 `E=None`（按空格理解）。
- issues:
  1. （无阻断）下次 A–D 对照请打「当前 vs 动手前备份」至少 5 格实值，不要只写总数/算术。点名货架号请用该 NO 自己的货架。
...
- plan_verdict: **approved**
- plan_checked_at: 2026-09-17 22:10
- plan_notes: |
    主表加列任务，不要求逐行识图/发票词组检索（与 round27 列6、round36 列7 同一口径）。round47 已写清列名/各 SHEET 列位/纳入范围/非死库存留空/不动既有列/系统操作列对齐/后续核销由豆包写，并已用 `_135732` 回退 1558 行，**不是空泛、YAML 已有 batch_size=20** → 批准方案，**不批准「一次性回填 1558（含 SHELF7=464 / SHELF1=4 / 中文未命中补标）」**。
    Cursor **选约束后的执行路径**（比照 round36：表头一次性 + 一条规则一次性，而不是全表组合信号）：
    - 「描述」表头加列 = 一次性（全部 19 个 SHEET，插在有效表头后一列；列号因结构不同：标准→col19、SHELF3→col13、SHELF5/6/7→col20）。从 `_135732` 或 `_回退后_20260917_214500` **空白加列**，禁止把 round45 那份已打 1558 的文件拷回来。
    - 回填规则收紧为**一条机械复制**：仅当该行**分类列**（Classification/分类）值含 `dead` 或 `死库存` 时，描述列写 `DEAD INVENTORY`。非死库存留空。淡蓝不参与判定（计划已写，同意）。
    - **中文品名「未命中」/「死库存」不单独打标**。SHELF4 那 35 行分类未更新 → 本批描述列留空（本计划写了不动既有列，不得顺手改分类）。若要填这 35 行须另交计划改分类列，或点名 NO 请用户拍板。
    - **待定行**即使中文含「未命中」也留空（分类列不是 dead 就不写）。
    - **只回填已核销柜** SHELF3 / SHELF4 / SHELF5 / SHELF6。SHELF7=464 历史柜、SHELF1=4、SHELF2/10-17/W7-ZA/W8-ZA：**只加列、现有行不回填**；后续核销时由豆包按货柜批写入。
    - 上述「分类列已是 dead → 描述列复制」视为一条规则一次性授权（比照列7 紧固件整表 CONSUMABLE）。交检须按 SHEET **写死实际条数**，禁止再报 1558。SHELF7/SHELF1 已填必须=0。
    - `batch_size=20` **不取消**。YAML 已写 20。以后新核销死库存行仍随货柜/小批写描述列，不得再开全表扫描。
    - 系统「是死库存」操作列与分类列本轮不改；豆包不改代码。字典空行补 F/E ≤20（避开 T.S.E.I. 等 9 词）可另批，不要夹在本任务。
- plan_issues:
  1. （无阻断，必须按此做）**第一批=加列 + 分类复制**：动手前新备份 `库存未匹配_备份_YYYYMMDD_HHMMSS.xlsx`（完整时间戳，禁止 `_<ts>`）。19 SHEET 加「描述」→ 只对 SHELF3/4/5/6 中分类列含 `dead`/`死库存` 的行写 `DEAD INVENTORY` → `batch_ready` / owner=cursor。禁止用中文品名补标；禁止回填 SHELF7/未核销柜现有行；禁止改既有列。
  2. （无阻断，交检必贴）Python `print(repr(...))` 贴进本文件，不要摘要、不要通配备份名：
     - 各纳入 SHEET 表头行 `repr`（必须看见「描述」及列位）；
     - 每 SHEET 描述列已填条数 **写死整数**（SHELF7=0、SHELF1=0、SHELF2/10-17/W7/W8=0）；全表合计写死一个整数（**禁止写 1558**）；
     - 抽样 ≥5 或约 25%（取较高者，可按柜分层抽）死库存行：SHEET | NO | 分类原值 | 中文品名 | 描述列 `repr`（分类须含 dead/死库存，描述须为 `DEAD INVENTORY`）；
     - 抽样 ≥5 非死库存行证描述列空（建议含：SHELF4 分类未更新的「未命中」行 ≥1、待定行 ≥1、SHELF7 任一行描述空）；
     - SHELF5 分类=dead 但无淡蓝的行：描述列仍可填（跟分类），注明条数；不要为了填描述去补淡蓝（不动既有列）；
     - 相对动手前备份：既有列变化=0；仅新增「描述」列。
  3. （无阻断）后续核销：命中/死库存判定仍走原硬规则；死库存行同步写描述=`DEAD INVENTORY`，与淡蓝+分类列并行。禁止用描述列反过来改判定。
  4. （无阻断）**修正范围**：只改 `库存未匹配.xlsx` 的描述列 + 把 repr 贴回 bridge；**禁止改字典**；**禁止改系统代码**；SHELF6 块5 继续挂起。改完 `status=batch_ready` / `owner=cursor`。
- next_action: **plan_approved**（owner=doubao。按上述约束执行后 `batch_ready`。字典空行补 F/E ≤20 可另批。`/dict` 筛选仍可在系统仓库推进。）
