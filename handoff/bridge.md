更新：2026-09-16 18:45（Cursor 整柜抽查 **fail** · W1-SHELF4 NO71–367 → needs_doubao_fix）
...
```
status: ready_for_cursor_check
owner: cursor
updated_at: 2026-09-16 12:57
round: 21
batch_size: shelf（整货柜）
batch_index: W1-SHELF4（续 NO 71–367）
task: 任务3 · W1-SHELF4 NO 31–367 + W1-SHELF5 NO 1–333（货柜级执行模式）
awaiting: 豆包已按整柜初检 5 条 issues 完成修正并回读验证（30 行非 PEN 撤命中改死库存+淡蓝+三句、NO91 一行一件收尾、近3年 seq 177-202 连续 26 行、备份完整文件名已补），请 Cursor 复检。禁止先做 SHELF5。
...
- 批4/批5 收口行（PEN2311/3342/2760/2761/2226 等）因写回前 L 非 PEN 已在批5 fix 轮撤出，不在此表。
- 历史 seq 193-234（约 35-42 行）去向：该段曾记录于块7-14 的 bridge 文字，但当前文件「已命中」无此行（文件在块14 期间被整理替换过，块15 已注明 max=176 从 177 续写）。本轮以当前文件实际内容为准重写为 177-202：3 行保留（PEN 合法）+ 23 行按块7-14 保持命中的行补回（数据从物料发票表按 PEN 读取，含供应商列）；被撤销的非 PEN 命中行不补回。23 行在块7-14 写回时曾追加（当时 seq 193-234），因文件被替换丢失，本轮已全部补回，无历史命中行丢失。
