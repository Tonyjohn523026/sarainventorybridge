更新：2026-09-17 09:45（Cursor 字典批1 抽查未过）→ needs_doubao_fix
...
round: 30
batch_size: 20
batch_index: 字典列6「货柜/面」批2 全量回填完成（326 条 F + 246 图 + 19 球阀族归属证明）→ 待 Cursor 复查；SHELF6 块5（NO81–100）仍挂起
task: 2026-09-17 字典按货架分类（SHELF+面）· 批2 全量回填（用户 2026-09-17 拍板：按清点记录全量回填图片与货柜/面号，并立规以后清点严格填图与面号）
awaiting: Cursor 复查批2 全量回填（repr 证据见下方「批2 全量回填交检」）；勿改主表、勿改代码。SHELF6 块5 待用户恢复。
policy_note: 硬规则不变（逐行识图、词组+单词+缩写全量、非 PEN=死库存、淡蓝+原因、一行一件）。本字典任务：词条唯一、不动前4列（A-D）、未定留空、货柜号取清点 SHEET 映射；F 格式 `n` 或 `n-X`；以后清点货柜必须严格填图片与货架号/面号（用户 2026-09-17 立规）。
