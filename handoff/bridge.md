...
round: 29
batch_size: 20
batch_index: 字典列6「货柜/面」批1 抽查未过 → 补交检实值/F格式/归属证明后同一批再 batch_ready；SHELF6 块5（NO81–100）仍挂起
task: 2026-09-17 字典按货架分类（SHELF+面）· 批1 返工（用户 2026-09-17 拍板按新规则更新字典）
awaiting: 豆包按上方 issues 补 Python repr 交检证据；F 写成 `2` 而非「货柜2」；证明 19 行确为 SHELF4 球阀族 31 词条否则撤回改对行。勿改主表、勿改代码。
policy_note: 硬规则不变（逐行识图、词组+单词+缩写全量、非 PEN=死库存、淡蓝+原因、一行一件）。本字典任务：词条唯一、不动前5列、未定留空、货柜号取清点 SHEET 映射；F 格式 `n` 或 `n-X`。
